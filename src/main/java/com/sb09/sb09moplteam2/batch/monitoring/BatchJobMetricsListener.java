...
public class BatchJobMetricsListener implements JobExecutionListener {
  ...
  public void afterJob(JobExecution jobExecution) {
    ...
    // 1. 커스텀 카운터 메트릭 (Builder 생성 비용 제거, Registry 직접 호출)
    meterRegistry.counter("mopl.batch.job.total",
            "job.name", jobName,
            "status", status.name())
        .increment();

    // 2. 커스텀 타이머 메트릭 (Builder 생성 비용 제거, Registry 직접 호출)
    if (jobExecution.getStartTime() != null && jobExecution.getEndTime() != null) {
      long durationMillis = Duration.between(jobExecution.getStartTime(), jobExecution.getEndTime()).toMillis();

      meterRegistry.timer("mopl.batch.job.duration",
              "job.name", jobName,
              "status", status.name())
      ...
      log.warn("[GLOBAL BATCH] Job 시작/종료 시간 정보가 누락되어 시간 메트릭을 적재하지 못했습니다.");
    }
  }
}
