...
class TmdbMovieProcessorTest {
  ...
  void process_이미_존재하는_콘텐츠는_null을_반환한다() {
    given(contentRepository.findAllExternalIdsByType(ContentType.movie))
        .willReturn(Set.of("1"));
  ...
  void process_새로운_tvSeries_콘텐츠를_생성한다() {
    given(contentRepository.findAllExternalIdsByType(ContentType.tvSeries))
        .willReturn(Set.of());
  ...
  @Test
  @DisplayName("이번 실행 중 같은 externalId가 다시 나오면 두번째부터 스킵된다")
